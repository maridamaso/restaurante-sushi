...
function verificarRoleUsuario() {
    ...
    if (user && user.role === "Cliente") {
        fetch(`${backendURL}/reviews?username=${user.username}`)
            ...
            .then((data) => {
                if (data.length > 0) {
                    document.getElementById("addReviewButton").style.display =
                        "none";
                    document.getElementById("editReviewButton").style.display =
                        "block";
                } else {
                    document.getElementById("addReviewButton").style.display =
                        "block";
            ...
            .catch((error) =>
                console.error("Erro ao verificar reviews do usuário:", error),
            );
    } else {
...
function mostrarPagina(pagina) {
    ...
    const paginasNavegacao = document.querySelectorAll(
        ".pagina-inicial, .cardapio, .reviews, .login, .editar-cardapio, .perfil",
    );
    ...
    var linkAtivo = Array.from(links).find(
        (link) => link.getAttribute("onclick") === `mostrarPagina('${pagina}')`,
    );
    if (linkAtivo) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (localStorage.getItem("isLoggedIn")) {
        ...
        if (
            linkAtivo &&
            linkAtivo.getAttribute("onclick") === "mostrarPagina('login')"
        ) {
            mostrarPagina("login");
        } else {
            document.querySelector(".perfil").style.display = "none";
...
// Função de registro
document
    .querySelector('.register-area button[type="button"]')
        ...
        .then((data) => {
            if (data.message === "Cadastro realizado com sucesso") {
                ...
                localStorage.setItem(
                    "user",
                    JSON.stringify({ email, username, role: data.role }),
                );
...
function mostrarFormularioReview(isEdit = false, reviewData = {}) {
    ...
    saveButton.onclick = () => {
        ...
        const review = {
            ...
            rating: ratingInputs.filter((input) => input.innerHTML === "⭐")
                .length,
